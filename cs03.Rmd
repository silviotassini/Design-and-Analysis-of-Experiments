...
t<-t.test(dados$Downstream, dados$Upstream,mu=0,alt="two.sided",paired=T)
...
```{r, echo=FALSE, fig.height=3.8, fig.cap="Figura 1 - Número de espécies por rio"}
...
plot(dados$Upstream,dados$Downstream,
     ylab="No Espécies depois",
     xlab="No Espécies antes",
     main="Dispersão entre no espécies antes e depois do ponto de despejo de esgoto")

abline(lm(dados$Upstream~dados$Downstrea),col="red")
```
...
Também foi definido um nível de significância de `r alpha*100`% ($\alpha=`r alpha`$) e uma diferença de no máximo `r d_star` espécies, $\delta^{*}$=`r d_star`. 
Uma vez que um mesmo individuo, no caso do estudo o rio, contribui com pares de amostras, coletadas antes e depois de um evento, a estatistica adequada para analise dos dados é o teste para amostras pareadas, pois é esperado que as observações no mesmo individuo sejam mais similares. 
...
indicador que, para um nível de confiança de `r (1-alpha)*100`%, a hipótese nula pode ser rejeitada
...
Para alcançar uma potência de $\beta = `r beta*100`$% seriam necessárias amostras de `r ceiling(ss$n)` rios. 

##Verificação das premissas

###Normalidade

A premissa de normalidade pode ser avaliada através do teste de Shapiro-Wilk. Utilizando as amostras obtidas, o resultado do teste foi um *p-valor* de `r round(normal$p.value,4)`, que indica que os dados seguem uma distribuição normal, como pode ser visto no gráfico.
...
Para avaliar a premissa de independência das variáveis foi realizado o teste de Durbin-Watson sobre a diferença entre as amostras obtidas acima e abaixo do local de despejo. O resultado foi um *p-valor* de `r round(indep$p.value,4)`, que prova a independência entre as amostras.
